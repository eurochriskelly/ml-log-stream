...
class LogParser {
    constructor(filename) {
        ...
        this.date = parts[parts.length - 3]
    ...
    logData = ({ line, lineNr }) => {
        if (this.mode === 'error') {
            ...
            const regex = /^(\S+) - (\S+) \[(\d{2}\/\w+\/\d{4}:\d{2}:\d{2}:\d{2} \+\d{4})\] '(\w+) (\S+) (HTTP\/\d\.\d)' (\d{3}) (\d+) - '(\S+)'/;
            const match = line.match(regex);
            if (match) {
                const [_, source, user, date, method, url, protocol, statusCode, response] = match;
                return this.record(
                    lineNr,
                    {
                        timestamp: date,
